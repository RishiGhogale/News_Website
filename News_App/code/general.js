fetch("https://newsapi.org/v2/top-headlines?country=us&category=general&apiKey=dfbd4ac4e6d142bbaf3d6f66936b3972")
  .then(response => response.json())
  .then(data => {
    let stories = data.articles.map(({ title, urlToImage, url,author, content, publishedAt,description }) => ({ title, author,urlToImage, url, content, publishedAt,description }))
