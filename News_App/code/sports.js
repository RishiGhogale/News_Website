fetch("https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=dfbd4ac4e6d142bbaf3d6f66936b3972")
  .then(response => response.json())
  .then(data => {
    let stories = data.articles.map(({ title, urlToImage, url, content, publishedAt,description,author }) => ({ title,author, urlToImage, url, content, publishedAt,description }))
